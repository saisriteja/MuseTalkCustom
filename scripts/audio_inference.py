...
class Avatar:
    ...
    def inference(self, 
        ...
        if out_vid_name is not None and args.skip_save_images is False: 
            ...
            output_vid = os.path.join(str(self.video_out_path), str(out_vid_name)+".mp4") # on
            cmd_combine_audio = f"ffmpeg -y -v warning -i {str(audio_path)} -i {str(self.avatar_path)}/temp.mp4 {output_vid}"
...
if __name__ == "__main__":
    ...
    # add a parser for audio clips
    parser.add_argument("--audio_clips_dir",
                        type=str,
                        default="data/audio",
    )
    ...
    # audio_clips = args.
    audio_clip_dir = args.audio_clips_dir
    from glob import glob

    audio_clips = glob(audio_clip_dir + "/*.wav")
    audio_clips.sort()
    audio_clips = {i: audio_clip for i, audio_clip in enumerate(audio_clips)}
